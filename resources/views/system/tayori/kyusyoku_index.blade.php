                ...
                <td class="text_title">
                	<div class="title_btn_area">
                        ...
                        <div class="title_edit_btn">
                            <a href="/school_newsletter/pdf/{{ $tayori->year }}/{{ sprintf("%02d", $tayori->month)}}/{{  $tayori->pdf }}" target="_blank">閲覧</a>
                        </div>
                        @endif
                        ...
                        @if($access)
                            ...
                            @else
                            <div class="title_edit_btn">
                                <a href="{{ route('kyusyoku.edit',$tayori->id)}}">編集</a>            
                            </div>
                            @endif
        
        
                            @if($tayori->status==\App\Tayori::PUBLISHED)
                                <form action="{{ route('kyusyoku.cancel',$tayori->id)}}" id="kyusyoku-cancel-{{$tayori->id}}" method="post">
                                    <input type="hidden" name="status" value="2">
                                    {{ csrf_field() }}
                                </form>
                                <div class="title_delivery_btn"> 
                                    <a href="" onclick="event.preventDefault();document.getElementById('kyusyoku-cancel-{{$tayori->id}}').submit();">取消</a>
                                </div>
                            @else
                                <form action="{{ route('kyusyoku.publish',$tayori->id)}}" id="kyusyoku-publish-{{$tayori->id}}" method="post">
                                    <input type="hidden" name="status" value="1">
                                    {{ csrf_field() }}
                                </form>
                                <div class="title_delivery_btn"> 
                                	<a href="" onclick="event.preventDefault();document.getElementById('kyusyoku-publish-{{$tayori->id}}').submit();">配信</a>
                                </div>
                            @endif        
                        @endif
                    </div>
...
<div class="eds02_01_btn_area">
    <div class="post_btn">
        <a href="{{route('kyusyoku.create')}}">新規作成</a>
    </div>
</div>
@endif
